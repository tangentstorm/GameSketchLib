...
*** quick recap/preview
this is week 2, lesson 2 of the GameSketch beginning game development series
this week, we're working our way up to a space invaders clone (show)
so far we've got some squares that know about overlap detection (show)
today, we'll use that samee logic to check
whether we're hit the squares when we shoot them with these bullets
*** let's look at the code we've written so far
**** > show OverlapLive.pde in eclipse
